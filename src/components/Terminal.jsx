...
const FS = {
  ...
  "/home/arthur": {
    type: "dir", url: "/",
    children: ["projects", "swift", "blog", "lab", "README.md", ".profile", ".secrets", ".bashrc"],
  },
  ...
  // ── PROJECTS ──
  "/home/arthur/projects": { type: "dir", url: "/projects", children: ["beatmap", "rptext", "file-organiser", "devboard"] },
  ...
  "/home/arthur/projects/file-organiser": { type: "dir", children: ["main.py", "README.md"] },
  "/home/arthur/projects/file-organiser/main.py": {
    type: "file",
    content: ["#!/usr/bin/env python3", '"""Auto-sorts downloads by type and date."""', "", "import os, shutil, time", "from watchdog.observers import Observer", "", "# watches ~/Downloads and sorts files", "# into folders by extension and date"],
  },
  "/home/arthur/projects/file-organiser/README.md": { type: "file", content: ["# file-organiser", "Python script that watches downloads and auto-sorts by type and date."] },
  "/home/arthur/projects/devboard": { type: "dir", children: ["index.ts", "README.md"] },
  "/home/arthur/projects/devboard/index.ts": { type: "file", content: ['import { serve } from "cloudflare-workers";', "", "// personal dashboard", "// aggregates GitHub activity, tasks, and notes"] },
  "/home/arthur/projects/devboard/README.md": { type: "file", content: ["# devboard", "Personal dashboard in TypeScript/React. Deployed on Cloudflare."] },

  // ── SWIFT ──
  "/home/arthur/swift": { type: "dir", url: "/swift", children: ["projects", "blog"] },
  "/home/arthur/swift/projects": { type: "dir", url: "/swift/projects", children: [] },
  "/home/arthur/swift/blog": { type: "dir", url: "/swift/blog", children: [] },

  // ── BLOG ──
  "/home/arthur/blog": { type: "dir", url: "/blog", children: ["posts"] },
...
// Build reverse URL → filesystem path map
const URL_TO_PATH = { "/": HOME };
Object.entries(FS).forEach(([path, node]) => {
  if (node.url && node.url !== "/") URL_TO_PATH[node.url] = path;
});
...
function saveState(lines, history) {
  try {
    const capped = lines.length > 300 ? lines.slice(-300) : lines;
    sessionStorage.setItem(SK_LINES, JSON.stringify(capped));
    sessionStorage.setItem(SK_HIST, JSON.stringify(history.slice(0, 50)));
  } catch {}
}

function loadState() {
  try {
    const l = sessionStorage.getItem(SK_LINES);
    const h = sessionStorage.getItem(SK_HIST);
    if (l) return { lines: JSON.parse(l), history: h ? JSON.parse(h) : [] };
  } catch {}
  return null;
}
...
export default function Terminal() {
  ...
  const handleCommand = (raw) => {
    ...
    if (base === "open") {
      ...
      if (!target) {
        ...
        if (node && node.url) {
          const currentPage = window.location.pathname.replace(/\/+$/, "") || "/";
          if (node.url === currentPage) { out([{ text: "you're already here.", color: "#475569" }]); return; }
          const newLines = [...linesRef.current, prompt, { text: `opening ${node.url}...`, color: "#34d399" }];
          saveState(newLines, [raw, ...histRef.current]);
          setTimeout(() => { window.location.href = node.url; }, 400);
    ...
    if (base === "tree") {
      ...
      function walk(p, pfx) {
        const n = FS[p]; if (!n || n.type !== "dir") return;
        const items = (n.children || []).filter((i) => !i.startsWith("."));
        items.forEach((item, i) => {
          const last = i === items.length - 1;
          const cp = p === "/" ? "/" + item : p + "/" + item;
          const cn = FS[cp]; const isDir = cn && cn.type === "dir";
          tl.push({ text: pfx + (last ? "└── " : "├── ") + item + (isDir ? "/" : ""), color: isDir ? "#60a5fa" : "#94a3b8" });
  ...
  const handleKeyDown = (e) => {
    if (e.key === "Enter") { e.preventDefault(); handleCommand(input); setInput(""); }
      ...
      if (matches.length === 1) {
        const pp = input.split(/\s+/); pp[pp.length - 1] = matches[0];
        const cp = cwd === "/" ? "/" + matches[0] : cwd + "/" + matches[0];
        const cn = FS[cp]; if (cn && cn.type === "dir") pp[pp.length - 1] += "/";
        setInput(pp.join(" "));
  ...
  return (
    <>
      <style>{`
        .term-tab{position:fixed;bottom:0;right:32px;z-index:9999;display:flex;align-items:center;gap:8px;padding:8px 18px 8px 14px;background:#111118;border:1px solid #1e1e2e;border-bottom:none;border-radius:8px 8px 0 0;cursor:pointer;transition:background .15s;box-shadow:0 -4px 20px rgba(99,102,241,.08);user-select:none}
        ...
        .term-btn{width:12px;height:12px;border-radius:50%;flex-shrink:0;border:none;cursor:pointer;padding:0;transition:filter .15s}.term-btn:hover{filter:brightness(1.25)}
        .term-btn-close{background:#ff5f57}.term-btn-min{background:#febc2e}.term-btn-max{background:#28c840}
        .term-btn-group{display:flex;gap:7px;align-items:center}
        .term-titlebar-label{position:absolute;left:50%;transform:translateX(-50%);font-family:'JetBrains Mono',monospace;font-size:11px;color:#334155;letter-spacing:.05em;pointer-events:none;white-space:nowrap}
        .term-body{padding:18px 20px 4px;height:320px;overflow-y:auto;scrollbar-width:thin;scrollbar-color:#1e293b transparent;flex-shrink:0}
      ...
      {!isClosed && !isMin && (
        <div className={`term-window${isMax ? " maximized" : ""}`} onClick={() => inputRef.current?.focus()}>
          ...
          <div className="term-body" ref={bodyRef}>
            {lines.map((l, i) => <div key={i} className="term-line" style={{ color: l.color || "#94a3b8" }}>{l.text}</div>)}
          </div>
